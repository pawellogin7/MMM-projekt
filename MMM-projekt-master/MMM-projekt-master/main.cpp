...
//	double F2pp = 0;
	...
	switch(sygnal_typ)
	{
		...
		{
			for ( int i = 0 ; i < granica ; i++ ){
				U[i] = sygnal_amp*sin(((i*2*M_PI*liczba_okresow)/1000));
			}
			break;
		}
	}
...
//		X1[i] = X1[i-1] + krok*( F1pp + 4*F1p + F1 )/3.0;
		X1[i] = X1[i-1] + krok*(F1p+F1)/2.0;
		
//		F2pp = F2p;
		F2p = F2;
		F2 = (1/(R2*C2))*X1[i-1] - (1/(R2*C2))*X2[i-1];
		X2[i] = X2[i-1] + krok*(F2p+F2)/2.0;
...
//		X2[i] = X2[i-1] + krok*( F2pp + 4*F2p + F2 )/3.0;
	...
	SetWindowText( max_amp_x2, GetAmpValue(MaxAmp_x2) );
	SetWindowText( time_x2, GetValue(czas_symulacji, 't', 0) );
	if(m1_x2 == true)
	{
		SetWindowText( x2_amp_marker1, GetAmpValue( MaxAmp_x2 * (805 - m1_x2_y) / 250 ) );
		SetWindowText( x2_czas_marker1, GetValue( (czas_symulacji * (m1_x2_x - 151) / 1000), 't', 1) );
	}
	if(m2_x2 == true)
	{
		SetWindowText( x2_amp_marker2, GetAmpValue( MaxAmp_x2 * (805- m2_x2_y) / 250 ) );
